...
public class deleteEmployeeScreenControl { 
    ...
    private void loadEmployeeList() {
        ...
        try (Connection connection = DriverManager.getConnection(Main.URL, Main.USER, Main.PASSWORD)) {
        	String query = "SELECT CONCAT(lname, ', ', fname) AS employee_name, isActive "
        			+ "FROM Employee WHERE isActive = TRUE ORDER BY lname;"; 
    ...
    private void handleDeleteEmployeeButtonAction() {
        ...
        try (Connection connection = DriverManager.getConnection(Main.URL, Main.USER, Main.PASSWORD)) {
            try (Statement stmt = connection.createStatement()) {
                String query = "UPDATE Employee SET isActive = FALSE WHERE lname = '" + lname + 
                		"' AND fname = '" + fname + "';";
